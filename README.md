...
| Asymmetric RelDiff Local | 触发几何同 Asymmetric RelDiff，但**小仓单独触发不再全局重置**：被触发的小仓只与**主仓（≥6%）**做保 NAV 的份额转移，其他未触发的小仓保持不变。补足（欠配）从超配主仓按相对偏离**由高到低**取份额、每个主仓**不被取到低于其基准权重**（不够则级联到下一个）；削减（超配）给欠配主仓按相对偏离由高到低补份额、每个主仓**不被填到高于其基准权重**（装不下则级联到下一个）。可用主仓不足时小仓仅**部分修正**（限额规则优先于回到目标）。主仓触发时仍全局重置 |
| Asymmetric RelDiff Local (Equal) | 同上，但小仓修正金额在**全部主仓间美元等额平分**，不设"不越过主仓基准比例"的限额（主仓可被推过/拉破自身目标），因此小仓总能**完全**回到目标。取份额侧主仓不可取穿至负值，不足部分级联给其余主仓 |
| Asymmetric RelDiff Local (Prop) | 同上，但小仓修正金额按主仓**当前市值比例**分摊（等比取/给），同样不设主仓目标限额，小仓总能完全回到目标 |
